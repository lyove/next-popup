...
Next-Popup 是一个轻量级、简单的弹出框工具库，可用于Poopver、Tooltip、Dropdown等，没有任何依赖，且支持Typescript。
...
Popup 会通过 `animationClass` 添加下面 6 个类。
...
Popup 提供了丰富的钩子函数，可以在弹框的各个生命周期执行代码。
...
| `wrapperClass` | `string` | | `popupWrapper` 自定义class |
...
| 参数 | 类型 | 描述 |
| -- | -- | -- |
| `config` | `PopupConfig` | Popup 配置参数 |
| `popupRoot` | `HTMLElement` | 弹框根元素 |
| `popupWrapper` | `HTMLElement` | 弹框元素 |
| `popupContent` | `HTMLElement` | 弹框内容元素 |
